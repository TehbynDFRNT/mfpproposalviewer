 ...
 * Provides functions for tracking events via the Analytics API
 */
...
/**
 * Send an event to analytics API
...
export async function trackEvent(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Failed to send event to analytics API:', response.status, errorData);
      return false;
    }
...
/**
 * Identify a user to analytics API
...
export async function identify(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ message: 'Unknown error' }));
      console.error('Failed to send identify event to analytics API:', response.status, errorData);
      return false;
    }
    
    return true;
  } catch (error) {
    console.error('Network error sending identify event to analytics API:', error);
    return false;
  }
}
