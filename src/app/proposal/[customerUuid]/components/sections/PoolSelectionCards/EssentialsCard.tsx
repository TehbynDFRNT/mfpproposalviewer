...
interface EssentialsCardsProps {
  fixed: ProposalData['poolSelection']['fixedCosts'];
}

export function EssentialsCards({ fixed }: EssentialsCardsProps) {
  // Essential items with benefits
    ...
    >
      <div className="flex flex-col space-y-6">
        <p className="text-white text-sm mb-4">
          Your pool comes with everything you need to start swimming. These essentials ensure your pool is ready to use from day one with top-quality equipment and features.
        </p>
        ...
        <Card className="w-full overflow-y-auto shadow-lg">
          <CardContent className="p-5 space-y-4">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Included in your package at no extra cost
            </p>
            ...
            <div className="flex justify-between items-baseline">
              ...
              <p className="text-xl font-bold">${fixed.reduce((sum, item) => sum + item.price, 0).toLocaleString()}</p>
            </div>
          </CardContent>
        </Card>
      </div>
