    ...
    <div className="relative flex min-h-screen flex-col bg-[#07032D] proposal-background"> {/* Changed to flex-col for header/main/footer layout */}
        ...
        <div className="order-1 lg:order-2 w-full lg:w-2/3 h-[40vh] lg:sticky lg:top-16 lg:h-[calc(100vh-8rem)] flex flex-col items-center justify-center overflow-hidden touch-none proposal-right bg-[#07032D] proposal-background transition-colors duration-300">
          ...
          <AnimatePresence mode="wait" onExitComplete={resetScroll}>
            ...
            {activeSection === CATEGORY_IDS.CUSTOMER_INFO && (
              <motion.div
                key="map"
                variants={{ ...fadeOut, ...visualIn }} /* same exit, earlier enter */
                initial="initial"
                animate="enter"
                exit="exit"
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-50 flex h-16
        ...
        <div className="absolute top-0 left-0 right-0 lg:hidden
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" className="rounded-full bg-white" onClick={() => { if (SM.canGoPrev(machineState)) { lastDir.current=-1; setDir(-1); setMachineState(SM.prev(machineState)); } }} disabled={!SM.canGoPrev(machineState)}>
              <ChevronUp className="h-5 w-5 text-[#DB9D6A]" />
            </Button>
            <Button variant="outline" size="icon" className="rounded-full bg-white" onClick={() => { if (SM.canGoNext(machineState)) { lastDir.current=1; setDir(1); setMachineState(SM.next(machineState)); } }} disabled={!SM.canGoNext(machineState)}>
              <ChevronDown className="h-5 w-5 text-[#DB9D6A]" />
            </Button>
          </div>
        </div>
