...
/**
 * File: src/components/Drawings/SimpleFooter.tsx
 * Styled footer for the upload portal that matches the proposal footer's appearance
 */
import { useState } from 'react';
import type { ProposalSnapshot } from '@/types/snapshot';
import { enableRenderReady, disableRenderReady } from '@/lib/renderStatusAPI';
...
import { ConfirmToggleDialog } from "@/components/Drawings/ConfirmToggleDialog";
import { SuccessDialog } from "@/components/Drawings/SuccessDialog";
...
export default function SimpleFooter({
  ...
  const navigateToProposal = () => {
    // The customerUuid in the URL is actually the project ID from the snapshot
    // We can use the same ID to navigate to the proposal page
    if (snapshot.project_id) {
      router.push(`/proposal/${snapshot.project_id}`);
    }
  };
  ...
  return (
    <footer className="fixed bottom-0 left-0 right-0 z-50 flex h-16
      ...
      {/* View Proposal button (left side) */}
      <Button
        variant="outline"
        size="lg"
        onClick={navigateToProposal}
        className="border-[#DB9D6A] text-[#DB9D6A] hover:bg-[#DB9D6A]/10"
      >
