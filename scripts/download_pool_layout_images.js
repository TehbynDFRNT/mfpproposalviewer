...
const OUTPUT_DIR = path.join(__dirname, '../public/Pools/Pool-Layout/Raw');
...
async function downloadAllImages() {
  ...
  poolData.pools.forEach((pool) => {
    ...
    const fileExtension = path.extname(pool.layoutImageUrl);
    const safeFileName = `${poolName.toLowerCase().replace(/\s+/g, '-')}_layout${fileExtension}`;
    
    downloads.push(downloadImage(pool.layoutImageUrl, safeFileName));
  });
  
  try {
    await Promise.all(downloads);
    console.log('All layout images downloaded successfully!');
  } catch (error) {
