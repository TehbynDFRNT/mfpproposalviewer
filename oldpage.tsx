...
const handleWheel = useCallback((e: React.WheelEvent) => {
  ...
  // ⬇︎ 1) Abort on tablets / phones – we don't want wheel‑based section
  //     navigation below the lg (1024 px) Tailwind breakpoint.
  //     Touch‑scroll inside the column will work as usual because
  //     we're not calling preventDefault().
  if (typeof window !== 'undefined' && window.innerWidth < 1024) return;

  const { deltaY } = e;

  // 3. ignore micro‑movements
  if (Math.abs(deltaY) < MIN_DELTA) return;

  // 4. ignore if we're still in cooldown
  if (scrollLock.current) { e.preventDefault(); return; }

  // 5. decide direction   (+1 = down, -1 = up)
...
const handleSectionSelectChange = useCallback((newSectionId: string) => {
  ...
  return (
    <div className="relative flex min-h-screen flex-col"> {/* Changed to flex-col for header/main/footer layout */}
      ...
      <main className="flex flex-col lg:flex-row flex-1 lg:pt-16 lg:pb-16">
        {/* Left Column - Scrollable Content */}
        <div ref={scrollColumnRef} onWheel={handleWheel} className="order-2 lg:order-1
      w-full lg:w-1/3 flex-1 sticky top-16 h-[calc(100vh-8rem)] overflow-y-auto"> {/* Ensure relative positioning */}
          ...
          <div className="sticky bottom-0      /* stick inside column */
                       hidden lg:block        /* only on desktop */
                       left-0 right-0 p-4 pt-2
                       backdrop-blur-sm border-t border-border/50
                       z-20 shadow-sm transition-all duration-200">
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2">
                <span className="text-base font-bold text-[#DB9D6A]">Skip to:</span>
                <Select value={activeSection} onValueChange={handleSectionSelectChange}>
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" size="icon" className="rounded-full bg-white"
                        onClick={() => { if (SM.canGoPrev(machineState)) { setDir(-1); lastDir.current=-1; setMachineState(SM.prev(machineState)); } }}
                        disabled={!SM.canGoPrev(machineState)}>
                  <ChevronUp className="h-5 w-5 text-[#DB9D6A]" />
                </Button>
                <Button variant="outline" size="icon" className="rounded-full bg-white"
                        onClick={() => { if (SM.canGoNext(machineState)) { setDir(1); lastDir.current=1; setMachineState(SM.next(machineState)); } }}
      ...
      overflow-hidden">
          ...
          <div className="hidden lg:block absolute bottom-4 right-4 z-50 bg-background/90 backdrop-blur-sm text-foreground p-4 rounded-lg shadow-lg border border-[#DB9D6A]/20 w-[28rem] overflow-hidden">
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-50 flex h-16
                     ...
                     bg-[#F9F4F0] px-4 md:px-6">
        {/* ——— mobile Skip‑to bar ——— */}
        <div
          className="absolute top-0 left-0 right-0  lg:hidden
                     -translate-y-full             /* pull it exactly one bar‑height upward */
